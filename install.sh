...
#               token.sh, inspect.sh, health.sh, help.sh, and the engine
#               itself, ./stack/core).
...
# compose.yml. Modules downloaded from the Garden are static files (see
...
# compose.yml/module.sh; the Core does not pre-seed module-specific
# variables here. ---
...
# by ./sprout from modules/<name>/compose.yml.
# ------------------------------------------------------------------------------
...
# READY or provisioning module, sourced verbatim from modules/<name>/compose.yml
# (modules without their own container, e.g. a model bootstrap module, define
# no compose.yml and contribute nothing here).
# ==============================================================================
services: {}
EOF_MODULES_COMPOSE
...
# download_module <module> — fetches module.yaml/compose.yml/module.sh from
...
regenerate_modules_compose() {
  ...
  for module_dir in "${MODULES_DIR}"/*/; do
    ...
    if [ "${status}" != "NOT_INSTALLED" ] && [ -f "${MODULES_DIR}/${module}/compose.yml" ]; then
...
log_info "Helper scripts created (start/stop/restart/recreate/logs/auth/token/inspect/health/help)"
